...
def generate_crt(issuerkey,key,path,issuerCN,CN):

    if not Path(path).exists():
        Path(path).mkdir()

    # key=generate_key(path)
    generate_csr(key,path,CN)
    ...
    issuer = x509.Name([
        ...
        x509.NameAttribute(NameOID.ORGANIZATION_NAME, u"Whatever"),
        # x509.NameAttribute(NameOID.COMMON_NAME, u"WhateverItIs.cuc.edu.cn"),
        x509.NameAttribute(NameOID.COMMON_NAME, issuerCN),
    ])
    ...
    ).sign(issuerkey, hashes.SHA256(),default_backend)
...
# app = Flask(__name__)    
# app.run('0.0.0.0', debug=True, port=8100, ssl_context=("./leaf/crt.crt", './leaf/key.pem'))  
# 运行后
